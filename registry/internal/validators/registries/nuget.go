...
func ValidateNuGet(ctx context.Context, pkg model.Package, serverName string, cfg *config.Config) error {
	err := validateAndNormalizeBaseURL(&pkg, cfg)
	if err != nil {
		return err
	}
...
func validateAndNormalizeBaseURL(pkg *model.Package, cfg *config.Config) error {
	// Get the expected registry URL from config (custom or default)
	expectedRegistryURL := cfg.GetRegistryNuGetURL()

	if pkg.RegistryBaseURL == "" {
		pkg.RegistryBaseURL = expectedRegistryURL
	}
	...
	// Validate that the registry base URL matches the expected URL
	if pkg.RegistryBaseURL != expectedRegistryURL {
		return fmt.Errorf("registry type and base URL do not match: '%s' is not valid for registry type '%s'. Expected: %s",
			pkg.RegistryBaseURL, model.RegistryTypeNuGet, expectedRegistryURL)
	}
